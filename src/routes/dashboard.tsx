      ...
      <div className="px-4 md:px-8 py-6 md:py-8 max-w-7xl mx-auto space-y-6">
        <header className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              to="/school"
              className="px-5 py-3 rounded-2xl bg-white font-semibold shadow-soft hover:shadow-glow flex items-center gap-2"
            >
              <School2 className="w-4 h-4" /> স্কুল
            </Link>
            <Link
              to="/bani"
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full bg-brand-green/10 px-3 py-1 text-xs font-semibold text-brand-green">
                  <School2 className="h-3.5 w-3.5" /> {schoolLabel}
                </div>
                <h2 className="mt-3 text-2xl font-bold">Your school-linked learning home</h2>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Use the national ecosystem to register schools, create student IDs, search
                  content, and join competitions.
                </p>
              </div>
              <Link
                to="/students"
                className="inline-flex items-center gap-2 rounded-2xl bg-gradient-hero px-4 py-3 text-sm font-semibold text-white shadow-soft"
              >
                <GraduationCap className="w-4 h-4" /> Student ID
              </Link>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
              <DashLink to="/school" label="স্কুল" icon={School2} />
              <DashLink to="/search" label="Search" icon={Search} />
              <DashLink to="/competitions" label="Competitions" icon={Trophy} />
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-sm text-muted-foreground mt-1">
                ক্লাসভিত্তিক চ্যাট, নোট, পোল, আর স্টাডি রুম।
              </div>
            </Link>
            <Link
              to="/live-class"
            ...
            >
              ...
              <div className="text-sm text-muted-foreground mt-1">
                শুধু শিক্ষামূলক YouTube সার্চ লিংক।
              </div>
            </Link>
            <Link
              to="/bangladesh-map"
            ...
            >
              ...
              <div className="text-sm text-muted-foreground mt-1">
                Google Maps খুলে নদী, বিভাগ, আর স্থাপনা দেখো।
              </div>
            </Link>
            <a
            ...
            >
              ...
              <div className="text-sm text-muted-foreground mt-1">
                অফিশিয়াল পাঠ্যপুস্তক বোর্ডের লিংক।
              </div>
            </a>
          </div>
        </section>
          ...
          <div className="lg:col-span-2 glass rounded-3xl p-6">
            ...
            <ul className="space-y-2">
              {[
                ...
                >
                  <span
                    className={`w-6 h-6 rounded-full grid place-items-center text-xs ${task.done ? "bg-brand-green text-white" : "border-2 border-muted-foreground/30"}`}
                  >
                    {task.done ? "✓" : ""}
                  </span>
          ...
          <div className="space-y-6">
            <div className="glass rounded-3xl p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {user.badges.map((b) => (
                  <span
                    key={b}
                    className="px-3 py-1.5 rounded-full bg-gradient-sunny text-sm font-medium text-white shadow-soft"
                  >
                    🏅 {b}
                  </span>
                ))}
            ...
            <div className="glass rounded-3xl p-6">
              ...
              <div className="bg-gradient-blue rounded-2xl p-4 text-white">
                ...
                <Link
                  to="/live-class"
                  className="mt-3 inline-flex px-3 py-1.5 rounded-lg bg-white text-primary text-xs font-semibold"
                >
                  এখনই যোগ দাও
                </Link>
              </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {topSubjects.map((s) => (
              ...
              >
                <div
                  className={`${s.gradient} w-12 h-12 rounded-xl grid place-items-center text-2xl mb-3`}
                >
                  {s.emoji}
                </div>
          ...
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                বাংলাদেশি শিক্ষার্থীদের জন্য দ্রুত স্টাডি লুপ।
              </p>
            </div>
...
function StatCard({
  ...
  return (
    <div className="glass rounded-2xl p-4 flex items-center gap-3">
      <div
        className={`${grad} w-11 h-11 rounded-xl grid place-items-center text-white shadow-soft`}
      >
        {icon}
      </div>
      <div>
...
function DashLink({
  ...
  return (
    <Link
      to={to}
      className="rounded-2xl border border-border bg-background/80 p-4 text-center shadow-soft hover:shadow-glow"
    >
