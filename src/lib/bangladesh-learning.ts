...
export const reels: ReelLink[] = [
  {
    title: "বাংলা বর্ণমালা রিভিশন",
    query: "Bangla alphabet educational shorts",
    classLevel: 1,
    subject: "বাংলা",
    emoji: "🔤",
  },
  {
    title: "সংখ্যার মজার ট্রিকস",
    query: "addition tricks for kids educational shorts",
    classLevel: 1,
    subject: "গণিত",
    emoji: "➕",
  },
  {
    title: "বাংলায় গল্প পড়া",
    query: "Bangla story reading for kids shorts",
    classLevel: 2,
    subject: "বাংলা",
    emoji: "📖",
  },
  {
    title: "গুণের টেবিল রিল",
    query: "multiplication table kids shorts",
    classLevel: 3,
    subject: "গণিত",
    emoji: "✖️",
  },
  {
    title: "বিজ্ঞানের মজার তথ্য",
    query: "science facts for children educational shorts",
    classLevel: 4,
    subject: "বিজ্ঞান",
    emoji: "🔬",
  },
  {
    title: "বাংলাদেশ তথ্য রিল",
    query: "Bangladesh facts for kids shorts",
    classLevel: 5,
...
const WORDS = {
  ...
  bdFacts: [
    ...
    [
      "Which day is Language Martyrs' Day?",
      "21 February",
      ["26 March", "16 December", "1 January"],
    ],
    [
      "Which is the largest mangrove forest in Bangladesh?",
      "Sundarbans",
      ["Sylhet", "Teknaf", "Kaptai"],
    ],
  ] as const,
};
...
const makeMathQuestion = (classLevel: ClassLevel, variant: number) => {
  ...
  if (classLevel === 2) {
    ...
    const { options, answer } = placeCorrect(
      String(correct),
      [String(correct - 1), String(correct + 1), String(correct + 3)],
      seed,
    );
    return {
      prompt: `What is ${a} - ${b}?`,
  ...
  if (classLevel === 3) {
    ...
    const { options, answer } = placeCorrect(
      String(correct),
      [String(correct - a), String(correct + b), String(correct + a)],
      seed,
    );
    return {
      prompt: `What is ${a} × ${b}?`,
...
const makeEnglishQuestion = (classLevel: ClassLevel, variant: number) => {
  ...
  if (classLevel === 3) {
    ...
    const { options, answer } = placeCorrect(
      correct,
      wrongs.length >= 3 ? wrongs : ["hot", "big", "fast"],
      seed,
    );
    return {
...
const categoryNames: QuizQuestion["subject"][] = [
  "গণিত",
  "ইংরেজি",
  "বাংলা",
  "বিজ্ঞান",
